...
render_guide('inspection_frequency')

with st.expander("❓ How This App Works (Click to Expand)", expanded=False):
    st.markdown("### What this app does\nUses example measurements, specifications, and process behavior to propose how often a process might be inspected. An interval is the time between inspections.\n\n### Step 1: Identify each process\nIn the sidebar, review the grouping columns. Each combination, such as part, machine, and cavity, gets its own analysis. Select only the Data columns containing actual numeric measurements; do not select identifiers as measurements.\n\n### Step 2: Check the standards mapping\nReview lower and upper specification limits, target, description, subgroup size, and range-chart type. A subgroup is a set of measurements taken together. The specification limits describe accepted product values; they are not control limits. Incorrect mapping changes the calculations.\n\n### Step 3: Choose the safety factor and run\nStart with the default safety factor of 2. A larger factor generally produces a shorter interval before the minimum and maximum bounds are applied. Click **Build inspection frequency** after changing settings.\n\n### Step 4: Read the results\n- **Inspection_Frequency:** Review each group's recommended interval and contributing metrics.\n- **Inspection_Matrix:** Compare the grouped recommendations.\n- **Standards_Coverage:** Check whether groups matched the standards.\n- **Raw Preview:** Inspect the example inputs.\nReview excluded groups rather than treating missing recommendations as permission to stop inspections. Groups with fewer than 15 subgroups are excluded; rule checks also need enough history.\n\n### Step 5: Export and validate\nDownload the workbook. Capability, drift, recorded failures, and rule flags inform a heuristic recommendation. Have a quality expert validate any production inspection schedule against process risk and customer requirements.\n\n### Demo boundaries and customization\nAll analysis uses built-in synthetic data. Explore the controls freely, then use the contact form at the bottom to describe your process and customization needs. Results are examples, not validated production decisions.\n")
st.caption("Select all columns explicitly • Maximum transparency")

with st.sidebar:
    st.header("📥 Load Data")
    mode = 'Use GitHub repo files'  # Demo-only deployment
    ...
    with col1:
        lo_spec_col = st.selectbox(
            ...
            index=([None] + std_cols).index('Lo Spec') if 'Lo Spec' in std_cols else 0,
        ...
        target_col = st.selectbox(
            "Target value",
            options=[None] + std_cols,
            index=([None] + std_cols).index('Target') if 'Target' in std_cols else 0,
        ...
        description_col = st.selectbox(
            ...
            options=[None] + std_cols,
            index=([None] + std_cols).index('Description') if 'Description' in std_cols else 0,
    ...
    with col2:
        hi_spec_col = st.selectbox(
            ...
            options=[None] + std_cols,
            index=([None] + std_cols).index('Hi Spec') if 'Hi Spec' in std_cols else 0,
        ...
        subgroup_size_col = st.selectbox(
            "Subgroup size",
            options=[None] + std_cols,
            index=([None] + std_cols).index('Subgroup size') if 'Subgroup size' in std_cols else 0,
        ...
        range_chart_col = st.selectbox(
            "Range chart type",
            options=[None] + std_cols,
            index=([None] + std_cols).index('Range chart type') if 'Range chart type' in std_cols else 0,
    ...
    safety_factor = st.number_input(
        ...
        value=2.0,
        step=0.25
    , help='Divides the proposed inspection interval: larger values generally mean more frequent checks, subject to interval bounds.')

    st.header("▶️ Run")
    run_btn = st.button("Build inspection frequency", type="primary", help='Calculate recommendations using the selected grouping, measurements, standards mapping, and safety factor. Run again after changing these settings.')
...
, help='Export the current inspection-frequency results and matrix to an Excel workbook for review.')
