...
if part_number_cols:
    part_col = st.sidebar.selectbox(
     ...
     help='Choose the column identifying the product or part type. Its values populate the part selection below.')
else:
    part_col = st.sidebar.selectbox("Select Part Number/SKU Column", options=all_cols, help='Choose the column identifying the product or part type when the app cannot automatically identify one.')
...
selected_part = st.sidebar.selectbox(
 ...
 help='Limit the analysis to the selected part number before building group summaries and anomaly scores.')
...
group_col = st.sidebar.selectbox(
    ...
    options=all_cols,
    index=0,
 help='Select the identifier that combines rows into a batch, lot, or work order. Each distinct identifier becomes one group for analysis.')

time_col_choice = st.sidebar.selectbox(
    "Timestamp (optional)",
    options=["—"] + all_cols,
    index=0,
 help='Choose the date/time field used for ordering when available, or select the dash if no timestamp applies.')
...
if layout == "Long format":
    meas_col = st.sidebar.selectbox(
        ...
        options=all_cols,
        index=0,
     help='For long-format reference data, select the column holding the numeric measurement values.')
elif layout == "Wide format":
    member_cols = st.sidebar.multiselect(
        "Member columns",
        options=all_cols,
        default=all_cols[:5] if len(all_cols) >= 5 else all_cols,
     help='For wide-format reference data, select the numeric subgroup measurement columns, such as Sample1 through SampleN. Avoid identifiers and text fields.')
...
include_last = st.sidebar.checkbox(
    ...
    value=False,
 help='Add each selected feature’s last value to its group summary. Confirm that record ordering represents the intended production sequence.')
...
top_n = st.slider(
    ...
    10, 300, 50,
 help='Control how many groups appear in the ranked display. This changes the display size, not the model’s anomaly threshold.')
...
st.download_button(
    ...
    mime="text/csv",
 help='Export the current group-level anomaly results as CSV. Anomaly scores describe unusualness, not a calibrated defect probability.')
...
chosen = st.selectbox(
 ...
 help='Select a group to inspect its feature profile and compare it with the reference groups in the analysis.')
