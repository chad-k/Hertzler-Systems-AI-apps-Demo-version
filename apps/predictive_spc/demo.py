...
with tab1:
    ...
    for idx, sim_feature in enumerate(sim_features):
        ...
        with sim_cols[idx]:
            sim_value = st.slider(
             ...
             help='Set a hypothetical value for this feature in the what-if simulation. Only model predictions change; no production equipment is adjusted.')
    ...
    with heat1:
        feature_x = st.selectbox(
            ...
            key="interaction_x",
         help='Choose the first input for the two-feature interaction view. Explore how the model response changes with combinations of inputs.')
    with heat2:
        ...
        feature_y = st.selectbox(
            ...
            index=0,
            key="interaction_y",
         help='Choose the second input to pair with feature X. Predicted relationships are model associations, not proof that changing a setting will cause that outcome.')
    ...
    st.download_button(
        ...
        mime="text/csv",
     help='Export the scored example records for the selected part and machine, including the available model risk outputs.')
