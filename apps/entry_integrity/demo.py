...
render_guide('entry_integrity')

with st.expander("❓ How This App Works (Click to Expand)", expanded=False):
    st.markdown('### What this app does\nChecks example measurement records for unusual entry patterns. It helps a reviewer find records to investigate; it does not decide whether an operator did anything wrong.\n\n### Step 1: Set the detection thresholds\nThe demo loads and maps its columns automatically. Use the sidebar to set the minimum time between entries, entries per minute, repeated-value window, low-variation threshold, and rounding threshold. Start with the defaults, then change one setting at a time to see its effect.\n\n### Step 2: Review the summary\nLook at the flagged-record and operator summaries. These are results of the configured rules, not a probability that a record is false. Lower timing thresholds and different rounding tolerances change which patterns qualify.\n\n### Step 3: Investigate the detail tabs\n- **Suspicious Records:** Filter by risk level and inspect the individual flags.\n- **Operator Summary:** Compare aggregated findings, keeping different workloads in mind.\n- **Flag Breakdown:** See which checks contributed to the findings.\n- **Batch Entry Analysis:** Explore records entered close together in time.\n- **Repeated Value Analysis / Low Variation Analysis:** Review repetition and small measurement spread.\n\n### Step 4: Export and discuss\nDownload the Excel report for a demo review. For a real implementation, agree on thresholds and legitimate exceptions with process owners. Automated collection, instrument resolution, and delayed entry can explain flags without misconduct.\n\n### Demo boundaries and customization\nAll analysis uses built-in synthetic data. Explore the controls freely, then use the contact form at the bottom to describe your process and customization needs. Results are examples, not validated production decisions.\n')
st.caption(
...
data_source = 'Use Demo Data'  # Demo-only deployment
...
fast_entry_seconds = st.sidebar.number_input(
    ...
    max_value=300,
    value=5
, help='Flag closely spaced entries using this time threshold in seconds. Rapid automated entry can be legitimate; investigate context.')
...
batch_entry_count = st.sidebar.number_input(
    ...
    max_value=100,
    value=10
, help='Set the count threshold for entries clustered within a minute. A flag may indicate batch entry, not necessarily an error.')
...
repeat_window = st.sidebar.number_input(
    ...
    max_value=50,
    value=10
, help='Number of consecutive records examined for repeated measurements. Change this to explore shorter or longer repetition patterns.')
...
low_variation_threshold = st.sidebar.number_input(
    ...
    value=0.001,
    format="%.6f"
, help='Standard-deviation threshold for unusually small spread. It uses the measurement units; mixed units and instrument resolution need careful interpretation.')
...
rounded_decimal_threshold = st.sidebar.number_input(
    ...
    max_value=6,
    value=0
, help='Set the decimal-precision cutoff for the rounding check. Whole numbers use zero decimals; legitimate instrument rounding may trigger it.')
...
with tabs[0]:
    ...
    risk_filter = st.multiselect(
        ...
        ["Low", "Medium", "High"],
        default=["Medium", "High"]
    , help='Show records with the selected rule-based risk levels. Filtering the display does not change the detection thresholds.')
...
with tabs[6]:
    ...
    , help='Download the integrity findings as an Excel workbook using the current demo analysis.')
