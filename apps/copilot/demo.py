...
render_guide('copilot')

with st.expander("❓ How This App Works (Click to Expand)", expanded=False):
    st.markdown('### What this app does\nTurns supported questions into charts and summaries of the built-in manufacturing measurements. It uses a command parser, so precise wording and the examples are useful.\n\n### Step 1: Choose your data scope\nUse the sidebar date range, part, machine, and characteristic controls. A characteristic is a property measured, such as a dimension. Start with a broad selection if a filter gives no records. Download current data to inspect the selected demonstration records.\n\n### Step 2: Ask a supported question\nExpand **Example questions** and try one of the displayed requests. Ask for a chart, capability, out-of-specification summary, comparison, or dashboard. Type in the question box and press **Send**. You can also use the microphone; check and edit its transcript before submitting.\n\n### Step 3: Check the interpreted request\nRead the response and chart labels to confirm the intended part, machine, characteristic, and date range. Naming entities in the request can change the selected scope. If the result is unexpected, simplify the request and use the sidebar filters.\n\n### Step 4: Interpret the answer\nA time chart shows behavior over time. Comparisons help you investigate differences. Cpk describes centering and spread relative to specifications; it is not a process-stability test. Out-of-specification results compare measurements with acceptance limits. A dashboard assembles several views. Ask explicitly when you want statistics or a data table.\n\n### Limits and next steps\nUnsupported wording may give a general summary rather than the chart you intended. The demo does not control machines, and voice availability depends on your browser and microphone permissions. Contact Hertzler to discuss your vocabulary, data, and useful production questions.\n\n### Demo boundaries and customization\nAll analysis uses built-in synthetic data. Explore the controls freely, then use the contact form at the bottom to describe your process and customization needs. Results are examples, not validated production decisions.\n')
...
with st.sidebar:
    ...
    selected_dates=st.date_input("Date range",value=(default_start,max_date),min_value=min_date,max_value=max_date, help='Limit the analysis to measurements within the selected dates. Expand the range if no records match.')
    ...
    part_filter=st.selectbox("Part",["All",*sorted(data["Part"].unique())], help='Choose the part to investigate. In Copilot, All keeps all parts within the other filters.')
    machine_filter=st.selectbox("Machine",["All",*sorted(data["Machine"].unique())], help='Choose the machine to investigate. In Copilot, All keeps all machines within the other filters.')
    characteristic_filter=st.selectbox("Trace / Characteristic",["All",*sorted(data["Characteristic"].unique())], help='Choose the measured quality characteristic to analyze, or keep All to explore the available characteristics.')
    st.download_button("Download current data",data=data.to_csv(index=False).encode("utf-8"),file_name="manufacturing_copilot_data.csv",mime="text/csv", help='Download the demonstration data matching the current sidebar filters.')
...
with mic_col:
    voice_text = speech_to_text(
        ...
        just_once=True,
        use_container_width=True,
        key="manufacturing_copilot_speech",
    )

# Put a new transcript into the editable text box.
if voice_text:
    st.session_state.copilot_question = voice_text

with input_col:
    with st.form("manufacturing_copilot_question_form", clear_on_submit=True):
        typed_question = st.text_input(
            "Question",
            key="copilot_question",
            placeholder=(
                "Ask for a chart, comparison, dashboard, statistics, "
                "or data table"
            ),
            label_visibility="collapsed",
         help='Enter a supported chart, comparison, capability, or dashboard request. Use Example questions for wording the parser understands.')
        submitted = st.form_submit_button(
            "Send",
            use_container_width=True,
         help='Submit the typed or edited voice-transcribed request for analysis. This does not send a contact email.')

question = typed_question.strip() if submitted and typed_question else None
