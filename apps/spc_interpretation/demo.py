...
with st.sidebar:
    ...
    n_subgroups = st.number_input("Number of subgroups", 20, 200, 60, step=5, help='Number of time-ordered measurement groups generated for the demo. Rule patterns require enough groups to be detectable.')
    subgroup_size = st.number_input("Subgroup size (n)", 2, 10, 5, step=1, help='Number of measurements in each subgroup. This affects the chart calculations and range interpretation.')
    seed = st.number_input("Seed offset", 0, 10000, 42, step=1, help='Change the random seed used for the synthetic example. This changes demo measurements, not real process data.')
    ...
    rule_set = st.selectbox(
        ...
        index=0
    , help='Select the pattern-detection rules. Custom exposes individual rule choices; the AIAG option is simplified.')
    # Defaults
    ...
    if rule_set == "Western Electric":
        ...
        selected = st.multiselect("Active rules", options=[f"R{r}" for r in range(1,9)], default=[f"R{r}" for r in range(1,9)], help='Choose which custom statistical pattern rules to evaluate.')
        active_rules = {r: (f"R{r}" in selected) for r in range(1,9)}
        params["run_length"] = st.slider("Run length for 'same side' (Rule 2)", 5, 12, 9, help='Number of consecutive subgroup means on the same side of the center required for this custom signal.')
        params["trend_length"] = st.slider("Trend length (Rule 3)", 5, 9, 6, help='Number of consecutive values required for a continuously increasing or decreasing custom trend signal.')
        params["alt_length"] = st.slider("Alternation length (Rule 4)", 10, 20, 14, help='Number of values required for the custom alternating up/down pattern.')
        params["r7_length"] = st.slider("Length for Rule 7 (within ±1σ)", 10, 20, 15, help='Number of consecutive values within one standard deviation of the center required for this custom pattern.')
        params["r8_length"] = st.slider("Length for Rule 8 (outside ±1σ)", 6, 12, 8, help='Window length used by the custom outside-one-standard-deviation pattern check.')

    st.header("Past Events (Demo)")
    use_demo_past = st.checkbox("Use demo past events", value=True, help='Include synthetic event-history examples when looking up possible causes and actions.')
    include_current_in_demo = st.checkbox("Demo includes current Part+Machine", value=True, help='Include this part-machine combination in demo history. Turn off to explore behavior when matching history is absent.')
    ...
    filter_mode = st.radio(
        ...
        index=2
    , help='Choose past-event explanations, generalized explanations, or their hybrid combination. This controls explanation sources.')

    st.header("Chart Options")
    show_sigma_bands = st.checkbox("Show 1σ/2σ bands", value=True, help='Display one- and two-standard-deviation bands around the chart center. These are statistical bands, not specification limits.')
    show_r_chart = st.checkbox("Show R chart", value=True, help='Display subgroup ranges (largest minus smallest measurement) to examine within-subgroup spread.')
    apply_filter_to_hover = st.checkbox("Apply filter mode to hover tooltips", value=True, help='Make hover explanations follow the chosen past/generalized/hybrid filter.')

st.title("SPC Auto-Interpretation")
from common.ui import render_guide
render_guide('spc_interpretation')

with st.expander("❓ How This App Works (Click to Expand)", expanded=False):
    st.markdown("### What this app does\nFinds selected control-chart patterns in synthetic measurements and explains possible causes and investigation actions. SPC means statistical process control.\n\n### Step 1: Choose a demonstration\nSelect a part and machine. Set the number of subgroups and measurements per subgroup in the sidebar. The seed offset changes the generated example. Larger datasets give more history to examine; a subgroup groups measurements collected together.\n\n### Step 2: Select the rules\nChoose Nelson, Western Electric, the simplified AIAG option, or Custom. Custom lets you choose active rules and pattern lengths. Different rule sets can flag different points. Changing a threshold changes the detection rule, not the underlying engineering specification.\n\n### Step 3: Choose explanation sources\nEnable demo past events to use the example event history. The current Part+Machine option controls whether that combination appears in the sample history. Past-only, generalized-only, and hybrid choices control which explanations are displayed. Use the hover-filter option to apply the same choice to chart tooltips.\n\n### Step 4: Read charts and interpretations\nHover over flagged subgroup points to inspect the signal and explanation. The 1-sigma and 2-sigma bands show distances from the estimated center in standard-deviation units. The R chart shows within-subgroup ranges. A pattern indicates something worth investigating; it does not establish a root cause. Past demo events are examples, not your factory's records.\n\n### Step 5: Download and follow up\nDownload the interpretation CSV. Distinguish observed rule signals from suggested causes and actions. A stable process can still be outside customer specifications, and a rule signal does not automatically mean defective product. Discuss appropriate rules and real event-history integration with Hertzler.\n\n### Demo boundaries and customization\nAll analysis uses built-in synthetic data. Explore the controls freely, then use the contact form at the bottom to describe your process and customization needs. Results are examples, not validated production decisions.\n")
...
with colA:
    selected_part = st.selectbox("Part", options=parts_known, index=0, help='Choose the part to investigate. In Copilot, All keeps all parts within the other filters.')
with colB:
    selected_machine = st.selectbox("Machine", options=machines_known, index=0, help='Choose the machine to investigate. In Copilot, All keeps all machines within the other filters.')
...
if interpret_rows:
    ...
    st.download_button(
        ...
        mime="text/csv",
     help='Export the current interpretation table as CSV for review of signals and suggested explanations.')
else:
