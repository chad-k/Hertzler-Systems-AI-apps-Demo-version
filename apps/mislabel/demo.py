...
- UPDATED: Add part number filters for scatter plot, PCA, and distribution
"""
...
with st.expander("❓ How This App Works (Click to Expand)", expanded=False):
    ...
    **Step 3: Explore Results**
    ...
    ### Feature Selection for Charts
    
    **Scatter Plot**: Uses first 2 selected features (X and Y axes)
    
    **PCA Chart**: Uses ALL selected features to compute principal components, displays PC1 vs PC2
    
    **Distribution Chart**: Shows first selected feature, grouped by part type
    ...
    - **Use the Part Number filters** in each chart to zoom in on specific part types
    """)
...
with col1:
    ...
    num_col = st.selectbox(
        ...
        cols,
        index=cols.index("DeclaredPartNo") if "DeclaredPartNo" in cols else 0,
...
with col2:
    ...
    feat_cols = st.multiselect(
        ...
        cols,
        default=[c for c in ["Meas1", "Meas2", "Temperature", "Pressure", "Speed"] if c in cols],
...
analysis_ready = st.button("▶ Analyze Data", type="primary", use_container_width=True, help='Run anomaly detection within each declared part type using the selected numeric features, then compare flagged records with other types. Suggestions require investigation; they do not verify a label.')
...
show_anom = st.checkbox("Show only anomalies", value=False, help='Restrict the displayed records to those flagged by the current analysis. This does not retrain the model.')
...
if len(feat_cols) >= 2 and len(display_df) > 0:
    ...
    # ===== NEW: FEATURE SELECTION FOR SCATTER PLOT =====
    scatter_feature_cols = st.columns(2)
    with scatter_feature_cols[0]:
        scatter_x_feature = st.selectbox(
            "X-Axis Feature (Scatter Plot)",
            feat_cols,
            index=0,
            key="scatter_x_feat"
        , help='Choose the numeric feature on the horizontal axis to compare it with the Y-axis feature.')
    with scatter_feature_cols[1]:
        scatter_y_feature = st.selectbox(
            "Y-Axis Feature (Scatter Plot)",
            feat_cols,
            index=min(1, len(feat_cols)-1),
            key="scatter_y_feat"
        , help='Choose the numeric feature on the vertical axis. Each plotted point represents an analyzed record.')
    
    # ===== PART NUMBER FILTER FOR SCATTER PLOT =====
    scatter_filter_cols = st.columns([2, 1])
    with scatter_filter_cols[0]:
        scatter_part_filter = st.multiselect(
            "Filter by Part Number (Scatter Plot)",
            sorted(df_analysis[num_col].unique()),
            default=[],
            key="scatter_filter"
        , help='Select the declared part types to include in this scatter plot. Choose at least one type to display points.')
    with scatter_filter_cols[1]:
        if st.button("Clear Filter", key="scatter_clear", help='Clear the part-type selections for this chart. Select the desired types again to populate its filtered view.'):
            st.session_state.scatter_filter = []
            st.rerun()
    
    # Apply filter
    scatter_display_df = display_df[display_df[num_col].isin(scatter_part_filter)]
    
    norm = scatter_display_df[scatter_display_df["AnomalyFlag"] == "Normal"].dropna(subset=[scatter_x_feature, scatter_y_feature])
    anom = scatter_display_df[scatter_display_df["AnomalyFlag"] == "Anomaly"].dropna(subset=[scatter_x_feature, scatter_y_feature])
    ...
    if len(norm_sampled) > 0:
        sns.scatterplot(data=norm_sampled, x=scatter_x_feature, y=scatter_y_feature, hue=num_col, 
                      ax=ax, palette="Set2", s=10, alpha=0.7, edgecolor="white", linewidth=0.5)
    if len(anom) > 0:
        ax.scatter(anom[scatter_x_feature], anom[scatter_y_feature], color='red', s=10, marker='X', 
                  edgecolors='darkred', linewidth=2, alpha=0.95, label='ANOMALY', zorder=5)
    
    ax.set_title(f"{scatter_x_feature} vs {scatter_y_feature} {sample_info}")
...
if len(feat_cols) >= 2 and len(display_df) > 1:
    ...
    # ===== NEW: PART NUMBER FILTER FOR PCA =====
    st.write("**Chart Uses:** All {} selected features → PC1 & PC2".format(len(feat_cols)))
    pca_filter_cols = st.columns([2, 1])
    with pca_filter_cols[0]:
        pca_part_filter = st.multiselect(
            "Filter by Part Number (PCA)",
            sorted(df_analysis[num_col].unique()),
            default=[],
            key="pca_filter"
        , help='Choose which declared part types to include in the PCA view. PCA summarizes several features on fewer axes; separation alone does not prove a mislabel.')
    with pca_filter_cols[1]:
        if st.button("Clear Filter", key="pca_clear", help='Clear the part-type selections for this chart. Select the desired types again to populate its filtered view.'):
            st.session_state.pca_filter = []
            st.rerun()
    
    # Apply filter
    pca_display_df = display_df[display_df[num_col].isin(pca_part_filter)]
    
    try:
        pca_data = pca_display_df[feat_cols].astype(float)
        ...
        if len(valid_rows) < 2:
            ...
            pca_df = pd.DataFrame({
                ...
                num_col: pca_display_df.loc[valid_rows.index, num_col].values,
                "Flag": pca_display_df.loc[valid_rows.index, "AnomalyFlag"].values
            })
...
if len(display_df) > 1 and len(feat_cols) > 0:
    ...
    # ===== NEW: FEATURE SELECTION FOR DISTRIBUTION =====
    dist_feature_cols = st.columns(1)
    with dist_feature_cols[0]:
        dist_feature = st.selectbox(
            "Feature to Display (Distribution)",
            feat_cols,
            index=0,
            key="dist_feat"
        , help='Choose the numeric measurement or parameter whose spread you want to compare between declared types.')
    
    # ===== PART NUMBER FILTER FOR DISTRIBUTION =====
    dist_filter_cols = st.columns([2, 1])
    with dist_filter_cols[0]:
        dist_part_filter = st.multiselect(
            "Filter by Part Number (Distribution)",
            sorted(df_analysis[num_col].unique()),
            default=[],
            key="dist_filter"
        , help='Choose which declared part types to compare in the distribution view for the selected feature.')
    with dist_filter_cols[1]:
        if st.button("Clear Filter", key="dist_clear", help='Clear the part-type selections for this chart. Select the desired types again to populate its filtered view.'):
            st.session_state.dist_filter = []
            st.rerun()
    
    # Apply filter
    dist_display_df = display_df[display_df[num_col].isin(dist_part_filter)]
    
    try:
        plot_data = dist_display_df[[num_col, dist_feature]].dropna()
        if len(plot_data) > 0:
            ...
            with col2:
                ...
                , help='Choose a box plot for median and spread, a violin plot for distribution shape, or a histogram for counts across value ranges.')
            
            with col1:
                fig, ax = plt.subplots(figsize=(14, 5))
                
                if plot_type == "Box Plot":
                    sns.boxplot(data=plot_data, x=num_col, y=dist_feature, hue=num_col, ax=ax, palette="Set2", legend=False)
                    ax.set_title(f"Box Plot - {dist_feature} by {num_col}")
                
                elif plot_type == "Violin Plot":
                    sns.violinplot(data=plot_data, x=num_col, y=dist_feature, hue=num_col, ax=ax, palette="Set2", legend=False)
                    ax.set_title(f"Violin Plot - {dist_feature} by {num_col} (better for large datasets)")
                
                else:  # Histogram
                    for ptype in sorted(plot_data[num_col].unique()):
                        data = plot_data[plot_data[num_col] == ptype][dist_feature]
                        ax.hist(data, alpha=0.5, label=ptype, bins=30)
                    ax.set_xlabel(dist_feature)
                    ax.set_ylabel("Count")
                    ax.set_title(f"Histogram - {dist_feature} by {num_col}")
                    ax.legend()
                ...
                stats_data = []
                for ptype in sorted(plot_data[num_col].unique()):
                    subset = plot_data[plot_data[num_col] == ptype][dist_feature]
                    stats_data.append({
                        "Type": ptype,
                        "Count": len(subset),
                        "Mean": subset.mean(),
