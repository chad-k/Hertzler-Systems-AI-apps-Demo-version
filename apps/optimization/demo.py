...
## How to use it
1. **Load synthetic demo files** using **Load / Refresh files**
...
with st.sidebar:
    ...
    tmpl_file = None  # Public demo has no upload control
    template = None
    ...
    mode = 'Use repo data (default)'  # Demo-only deployment

    data_up = specs_up = None
    st.code('data/synthetic_process_data.csv')
    st.code('data/spec_limits.csv')

    load_btn = st.button("Load / Refresh files", type="primary", key="load_btn", help='Reload the bundled synthetic process and specification files into this demo session.')
...
if load_btn or (data_df_ss is None) or (specs_df_ss is None):
    try:
        ...
        st.success("Synthetic demo files loaded. You can change the analysis settings.")
    except Exception as e:
...
, help='Save the current column mapping and parameter configuration as a JSON file for reference or reuse in a compatible version. This public demo does not accept template uploads.')
...
with btn_row[0]:
    run_btn = st.button("Run optimization", type="primary", key="run_opt", help='Fit the configured model and search within the selected parameter bounds for predictions close to the target, separately for each part-machine group.')
with btn_row[1]:
    clear_btn = st.button("Clear results", key="clear_results", help='Remove saved results from the session. Because automatic analysis is enabled, the app can immediately calculate fresh results from the current settings.')
...
with tabs[0]:
    ...
    if results_df.empty:
        ...
        filter_in_spec = st.checkbox("Show only in-spec results", value=False, key="filter_in_spec", help='Display only recommendations whose predicted measurement meets the specification test. This is a model prediction, not a production acceptance result.')
        ...
        with col1:
            st.download_button(
                ...
                mime="text/csv",
             help='Export all suggested settings and predicted outcomes from the current optimization. Validate candidates before changing a real machine.')
        with col2:
            ...
            , help='Export recommendations whose predicted measurement passes the specification check. The button is disabled when none qualify.')
...
with tabs[1]:
    ...
    if failures_df is None or failures_df.empty:
        ...
        st.download_button(
            ...
            mime="text/csv",
         help='Export the groups that could not produce an optimization result and their recorded reasons.')
